...
namespace RVisUI.Wpf
{
  public class AdornedControl : ContentControl
  {
    ...
    public static readonly DependencyProperty ShowAdornerDelayProperty =
      DependencyProperty.Register(
        "ShowAdornerDelay",
        typeof(int),
        typeof(AdornedControl),
    ...
    /// <summary>
    /// ms to delay adorning.
    /// </summary>
    public int ShowAdornerDelay
    {
      get
      {
        return (int)GetValue(ShowAdornerDelayProperty);
      }
      set
      {
        SetValue(ShowAdornerDelayProperty, value);
      }
    }
    ...
    private void ShowOrHideAdornerInternal()
    {
      if (IsAdornerVisible)
      {
        var delay = ShowAdornerDelay;
        if (delay > 0)
        {
          var popupTimer = new DispatcherTimer(DispatcherPriority.Normal)
          {
    ...
    private void ShowAdornerInternal()
    {
      ...
      if (this.AdornerContent != null)
      {
        ...
        if (this._adornerLayer != null)
        {
          this._adorner = new FrameworkElementAdorner(
            this.AdornerContent,
            this,
